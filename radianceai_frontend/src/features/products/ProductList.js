...
import supabase from "../../api/supabaseClient"; // assuming you have this or replace accordingly
...
/**
 * Helper to fetch brand info by array of brand_ids.
 * Replace this with your actual API or supabase call.
 */
async function fetchBrandsByIds(ids) {
  ...
  if (error) {
    console.error("[fetchBrandsByIds] Error fetching brands:", error);
    return [];
  }
  return data || [];
}
...
function ProductList() {
    ...
    fetchRecommendedProducts()
      .then(async (res) => {
        ...
        if (Array.isArray(res)) {
          // Shuffle and select first 6 products
          const shuffled = res.sort(() => 0.5 - Math.random());
          const selected = shuffled.slice(0, 6);

          // Extract unique brand_ids
          const uniqueBrandIds = [...new Set(selected.map((p) => p.brand_id))];

          // Fetch brands data
          const brandsData = await fetchBrandsByIds(uniqueBrandIds);
          ...
          // Attach brandName to each product
          const productsWithBrandNames = selected.map((p) => ({
            ...p,
            brandName: brandMap[p.brand_id] || "Unknown Brand",
          }));
      ...
      .catch((e) => {
        ...
        setError({
          message:
            "Unable to load products. Network/API error: " +
  ...
  return (
    <section
      className="container"
      style={{ margin: "0 auto 42px auto", maxWidth: 1180 }}
    >
      <h2
        style={{
          fontWeight: 900,
